(function (global) {
  ...
  class DynamicTitle {
    constructor(element, options = {}) {
      ...
      this._setup();
      this._start();                    // <-- varmistettu heti käyntiin
      this._resizeHandler = () => {
    ...
    // ... (kaikki muut metodit täysin samanlaisina kuin sinulla oli – _collectPhrases, _setup, rebuild, _buildDOM, _start, _tick, stop, _cleanupSpans, destroy, static init ja initAll) ...
    ...
    _setup() { /* sama kuin sinulla */ this.stop(); this.el.innerHTML = ''; this.el.style.position = this.el.style.position || 'relative'; this.el.style.overflow = 'visible'; this.el.setAttribute('aria-live', 'polite'); this.el.setAttribute('role', this.el.getAttribute('role') || 'status'); this._buildDOM(); }
    rebuild() { this.phrases = this._collectPhrases(); this._cleanupSpans(); this._setup(); this._start(); }
    _buildDOM() { /* täysin sama koodi kuin sinulla oli */ 
    ...
    _tick() { /* täysin sama kuin sinulla */ 
      const prev = this.current;
      const next = (this.current + 1) % this._items.length;
    ...
    _cleanupSpans() { /* sama */ if (this._items) { this._items.forEach(it => { if (it.measured && it.measured.parentNode) it.measured.parentNode.removeChild(it.measured); if (it.real && it.real.parentNode) it.real.parentNode.removeChild(it.real); }); } this._items = null; }
    ...
    static initAll() { const nodes = Array.from(document.querySelectorAll('.dynamic-title, [data-dynamic-title]')); return nodes.map(n => new DynamicTitle(n)); }
  }

  // ALWAYS PLAY + heti käyntiin (muutos täällä)
  function autoInit() {
    DynamicTitle.initAll();   // suoraan ilman fontti-odotusta tai setTimeout-viivettä
  }
